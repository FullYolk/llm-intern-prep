...
DATA_FILE = os.path.join(BASE_DIR,"students.json")##join函数是什么意思?
...
def delete_student(students: List[dict], name: str):
    for s in students:
        if s["name"] == name:
            students.remove(s)
            return
...
def sort_students(students: List[dict]) -> List[dict]:
    return sorted(students, key=lambda x: (x["score"], x["name"]), reverse=True)
